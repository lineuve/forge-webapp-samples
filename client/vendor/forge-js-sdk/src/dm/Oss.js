    ...
    Autodesk.Forge.OSS = {
        ...
        createBucket: function(bucketParams){
            ...
            if (bucketParams.bucketKey) {
                ...
                return Client.authorized2LeggedApiRequest(_apiEndpoint + '/buckets').post(headers, JSON.stringify(bucketParams));
            }
        ...
        getBucketDetails: function (bucketKey) {
            ...
            if (bucketKey) {
                return Client.authorized2LeggedApiRequest(_apiEndpoint + '/buckets/' + bucketKey + '/details').get();
            }
        ...
        downloadFile: function (bucketKey, objectName) {
            ...
            if (bucketKey && objectName) {
                return Client.authorized2LeggedApiRequest(_apiEndpoint + '/buckets/' + bucketKey + '/objects/' + objectName,{notJsonResponse:true}).get();
            }

            return Promise.reject(new Error('You need to supply bucketKey and objectName'));
        },

        createSignedURL: function(bucketKey, objectName){
            //Make sure fileId is defined and that it is valid
            if (bucketKey && objectName) {
                return Client.authorized2LeggedApiRequest(_apiEndpoint + '/buckets/' + bucketKey + '/objects/' + objectName + '/signed').post();
            }
