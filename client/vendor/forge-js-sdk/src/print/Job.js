...
(function () {
     ...
     * @constructor
     */
    Autodesk.Forge.Jobs = function (data) {
        Autodesk.Forge.Paginated.call(this, data);
    };

    Autodesk.Forge.Jobs.prototype = Object.create(Autodesk.Forge.Paginated.prototype);
    Autodesk.Forge.Jobs.prototype.constructor = Autodesk.Forge.Jobs;
     ...
     */
    Autodesk.Forge.Jobs.get = function (headers, params) {
        return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/jobs')
            .get(headers, params)
            .then(function (data) {
     ...
     * @returns {Promise} - A promise that will resolve to an array of jobs.
     */
    Autodesk.Forge.Jobs.getPrinter = function (printerId, headers, params) {
        return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/printers/' + printerId + '/jobs')
            .get(headers, params)
            .then(function (data) {
                return new Autodesk.Forge.Jobs(data);
            });
    ...
    Autodesk.Forge.Jobs.prototype.parse = function (data) {
        ...
        if (data) {
            ...
            if (Array.isArray(jobs)) {
                ...
                jobs.forEach(function (job) {
                    ...
                    _this.push(new Autodesk.Forge.Job(job));
                });
            }
        }
    };
     ...
     * @constructor
     */
    Autodesk.Forge.Job = function (data) {
        this.status = null;
        this.progress = 0.0;
    ...
    Autodesk.Forge.Job.prototype = {

        constructor: Autodesk.Forge.Job,
        ...
        getStatus: function () {
            ...
            var _this = this;
            return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/jobs/' + this.id)
                .get()
                .then(function (data) {
         ...
         * @see {@link Autodesk.Forge.TrayAPI.generatePrintable}
         */
        ...
        updateStatus: function (newStatus, newComment, customData) {
            ...
            return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/jobs/' + this.id + '?status=' + urlStatus + '&comment=' + urlComment)
                .put(headers, payload)
        ...
        setPrintable: function (printableId) {
            ...
            return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/jobs/' + this.id + '/printable')
        ...
        setPrinter: function (printerId) {
            ...
            return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/jobs/' + this.id + '/printer')
                .post(headers, payload)
                .then(function (response) {
                    if (response.printer_id) {
                        _this.printer_id = response.printer_id;
        ...
        setCallback: function (callbackUrl) {
            return Client.authorized3LeggedApiRequest(_apiEndpoint + '/print/jobs/' + this.id + '/register')
         ...
         * custom data in the property "Autodesk.Forge.source_tray". Although not required
        ...
        setSourceTray: function (trayId) {
            ...
            if (!this.data.job_custom_data.Autodesk.Forge) {
                this.data.job_custom_data.Autodesk.Forge = {};
            }
            this.data.job_custom_data.Autodesk.Forge.source_tray = trayId;
        ...
        getSourceTray: function () {
            ...
            if (custom && custom.Autodesk.Forge) {
                return custom.Autodesk.Forge.source_tray;
            }
            return null;
        },
         ...
         * with the job custom data in the property 'Autodesk.Forge.source_files'. Although
        ...
        setMeshPaths: function (meshPaths) {
            ...
            this.data.job_custom_data.Autodesk.Forge.source_files = meshPaths;
        ...
        getMeshPaths: function () {
            var custom = this.data.job_custom_data;
            if (custom && custom.Autodesk.Forge) {
                return custom.Autodesk.Forge.source_files;
            }
            return [];
        }
    };
